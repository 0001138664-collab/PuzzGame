...
// Função para desbloquear o áudio (necessário em alguns navegadores)
function unlockAudio() {
    ...
    // Fecha o pop-up de instruções
    document.getElementById('instructions').close();
}

// Adiciona um listener para tocar a música quando a página carregar
document.addEventListener('DOMContentLoaded', () => {
    // O áudio só será tocado após a interação do usuário (clique no botão "Close" do pop-up)
    // A função unlockAudio() é chamada no onclick do botão "Close" no index.html
});
...
// Sequência de teclas para ativar o easter egg: Cima, Cima, Baixo, Baixo, Esquerda, Direita, Esquerda, Direita, B, A
const konamiCode = [
    'ArrowUp', 'ArrowUp',
    'ArrowDown', 'ArrowDown',
...
let konamiIndex = 0; // Contador para rastrear o progresso na sequência

document.addEventListener('keydown', (event) => {
    // Converte a tecla para minúsculas para ser case-insensitive (útil para 'b' e 'a')
    const key = event.key.toLowerCase();

    // Verifica se a tecla pressionada corresponde ao próximo passo do Konami Code
    if (key === konamiCode[konamiIndex]) {
        konamiIndex++; // Avança para a próxima tecla na sequência
        
        // Se o código foi completado (o índice é igual ao tamanho da sequência)
        if (konamiIndex === konamiCode.length) {
            console.log("EASTER EGG ATIVADO! Parabéns, você encontrou o código secreto!");
            
            // --- AÇÃO DO EASTER EGG ---
            // 1. Mudar a cor de fundo do corpo da página para um tom vibrante (Ouro)
            document.body.style.backgroundColor = '#FFD700'; 
            
            // 2. Adicionar uma mensagem divertida e chamativa no topo da página
            const easterEggMessage = document.createElement('h1');
            easterEggMessage.textContent = 'GEOMETRY PUZZLE MASTER!';
            easterEggMessage.style.textAlign = 'center';
            easterEggMessage.style.color = '#8A2BE2'; // Azul Violeta
            easterEggMessage.style.marginTop = '50px';
            easterEggMessage.id = 'easter-egg-msg'; // ID para fácil remoção/verificação
            
            // Remove a mensagem anterior se o easter egg for ativado novamente
            const existingMsg = document.getElementById('easter-egg-msg');
            if (existingMsg) {
                existingMsg.remove();
            }
            
            // Adiciona a nova mensagem ao corpo, antes de qualquer outro elemento
            document.body.prepend(easterEggMessage);
            
            // 3. Opcional: Tocar um som de celebração (se houver um arquivo de som disponível)
            // Exemplo: new Audio('celebration.mp3').play();

            // Reinicia o índice para que o código possa ser ativado novamente
            konamiIndex = 0;
        }
    } else {
        // Se a tecla errada for pressionada, reinicia a sequência do zero
        konamiIndex = 0;
    }
});

// Fim da lógica do Easter Egg
// ====================================================================
